...
# Format: <seq serial num> <title> <len seq(aa)> <seq(aa)>
...
if len(argv[1:]) != 4:
    ...
    if (WeightFactor < 0.00) or (WeightFactor > 1.00):
        ...
        print 'Please select any value within the region from 0.05 to 0.70 for the weight factor.'
    else:
        fin=open(infile, 'r')
        fout=open(outfile, 'w')
        flog=open('input_seq.log', 'w')
        val1n=fin.read().split('>')
        ...
        if val1n[0] != '':
            ...
            for m in val1n[1:]:
                ...
                seq = string.join(sequence_lines, '')
                seq=seq.upper()
                seq=seq.replace('\r','')
                flog.write(str(seq_num)+'\t'+title+'\t'+str(len(seq))+'aa\t'+seq+'\n')
                ...
                if len(check_seq(seq)) != 0:
                ...
                elif (LambdaVal >= 0) and ((len(seq)-LambdaVal) > 0):
                    all_aa_freq=[]
                    sum_all_aa_freq=0
                    for aa in aa_20:
                        #normalized occurrence frequency of the 20 amino acids
                        aa_freq_val=float(aa_frequency(seq).get(aa, 0))/len(seq)
                    ...
                    denominator_val=sum_all_aa_freq+(WeightFactor*sum_all_theta_val)
 
                    all_PseAAC=[] # Eq. 5
                    for val1 in all_aa_freq:
                        all_PseAAC.append(str(round((val1/denominator_val),5)))
                    for val2 in all_theta_val:
                        all_PseAAC.append(str(round(((WeightFactor*val2)/denominator_val),5)))
 
                    #print all_PseAAC
                    fout.write(str(seq_num)+'\t'+string.join(all_PseAAC, ' ')+'\n')
                    seq_num+=1
                else:
                    print 'Lambda must be non-Negative integer, such as 0, 1, 2, ...'
                    print 'Lambda should NOT be larger than the length of input protein sequence.'
                    print 'when Lambda=0, the output of PseAAC is the 20-D amino acid composition.'
                    print
